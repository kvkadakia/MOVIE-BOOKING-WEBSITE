...
#bookin a:active{
	...
	<script>
		...
		function reserveSeats() {
			...
			if (selectedList) {
				if (confirm('Do you want to reserve selected seat/s ' + selectedList + '?')) {
					...
					document.forms[0].action='bookseats7.php';
		...
		function confirmSeats() {
			document.forms[0].action='Payment7.html';
					document.forms[0].submit();
		}
...
<h1>SCREEN 4 - Bahubali 12.30pm (U/A HINDI)</h1>
</div>
<br>
<div style="background-color:white">
...
@ mysql_select_db("tickets7") or die("Could not select database");
