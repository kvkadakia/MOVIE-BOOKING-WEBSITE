...
#bookin a:active{
	...
	<script>
		...
		function reserveSeats() {
			...
			if (selectedList) {
				if (confirm('Do you want to reserve selected seat/s ' + selectedList + '?')) {
					...
					document.forms[0].action='bookseats6.php';
		...
		function confirmSeats() {
			document.forms[0].action='Payment6.html';
					document.forms[0].submit();
		}
...
<h1>SCREEN 3 - Brothers 5.30pm (U/A HINDI)</h1>
</div>
<br>
<div style="background-color:white">
...
@ mysql_select_db("tickets6") or die("Could not select database");
