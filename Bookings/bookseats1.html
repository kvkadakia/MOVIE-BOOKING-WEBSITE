...
<?php
	...
	if (isset($_POST['seats']))
	{
		...
		@ mysql_select_db("tickets1") or die("Could not select database");

		// prepare select statement
		$selectQuery = "SELECT rowId, columnId from seats where (";
		/*$r=mysql_query($selectQuery);
		$seats = mysql_fetch_array($r);*/	
		
		$count = 0;
		$i=0; //Copy this
		//here the rows and columns are selected
		...
		if ($selectedSeats != $count) {
			...
			$problem .= "<a href='seats1.php'>View Seat Plan</a>";
		...
		echo "<a href='seats1.php'>View Seat Plan</a>";
		
		
		
		mysql_close();
		echo "<meta http-equiv='refresh' content='0;url=http://localhost/Project/Bookings/seats1.php' />";
	}
