...
#bookin a:active{
	...
	<script>
		...
		function reserveSeats() {
			...
			if (selectedList) {
				if (confirm('Do you want to reserve selected seat/s ' + selectedList + '?')) {
					...
					document.forms[0].action='bookseats1.php';
		...
		function confirmSeats() {
			document.forms[0].action='Payment1.html';
					document.forms[0].submit();
		}
...
<h1>SCREEN 1 - Phantom 12.30pm (U/A HINDI)</h1>
</div>
<br>
<div style="background-color:white">
...
while (list($rowId, $columnId, $status, $updatedby) = mysql_fetch_row($result))
{
	...
	echo "</td>";
		if (($rowId == 'A' && $columnId == 7) 
			...
			|| ($rowId == 'L' && $columnId == 13) 
			|| ($rowId == 'M' && $columnId == 9)) {
			// This fragment is for adding a blank cell which represent the exit
			echo "<td>&nbsp;</td>";
		}
}
